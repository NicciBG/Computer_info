...
static int get_core_topology(CPU_DATA* data) {
	...
	ok = GetLogicalProcessorInformationEx(RelationProcessorCore,
										  ...
										  &len);
	if (GetLastError() != ERROR_INSUFFICIENT_BUFFER) return 204;

	PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX buffer =
		malloc(len);
	if (!buffer) return 203;

	ok = GetLogicalProcessorInformationEx(RelationProcessorCore,
										  buffer, &len);
	if (!ok) { free(buffer); return 205; }
	...
	if (!data->cores) { free(buffer); return 203; }
...
static int get_core_topology(CPU_DATA* data) {
	...
	for (int cpu = 0; cpu < L; ++cpu) {
		...
		snprintf(path_phy, sizeof(path_phy),
		...
		int phy = 0;
		if (f) { fgets(buf, sizeof(buf), f); phy = atoi(buf); fclose(f); }

		f = fopen(path_core, "r");
		int core = 0;
		if (f) { fgets(buf, sizeof(buf), f); core = atoi(buf); fclose(f); }
	...
	if (!data->cores) { free(keys); free(temp); return 203; }
	...
	for (int cpu = 0; cpu < L; ++cpu) {
		...
		data->cores[idx].logical_ids[
			data->cores[idx].logical_count++] = cpu;
	}
...
static void populate_caches_and_freq(CPU_DATA* data) {
	...
	memset(data->l1size, 0,
		   data->logical_core_count * sizeof(int));
 ...
 */
static void parse_shared_cpu_list(const char* str,
...
static void populate_caches_and_freq(CPU_DATA* data) {
	...
	for (int idx = 0; ; ++idx) {
		...
		snprintf(levelp, sizeof(levelp),
				 "%s/level", dir);
		snprintf(sizep, sizeof(sizep),
				 "%s/size", dir);
		snprintf(sharep, sizeof(sharep),
				 "%s/shared_cpu_list", dir);

		FILE* f = fopen(levelp, "r");
		int level = 0;
		if (f) { fgets(buf, sizeof(buf), f); level = atoi(buf); fclose(f); }
		...
		f = fopen(sharep, "r");
		char listbuf[64] = { 0 };
		if (f) { fgets(listbuf, sizeof(listbuf), f); fclose(f); }
